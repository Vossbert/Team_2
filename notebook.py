...
def _():
    ...
    mrmm_model="mrm8488/t5-small-finetuned-text-simplification" # 60 Mio Parameters
    flan_t5_small="google/flan-t5-small" # 77 Mio Parameters
    flan_t5_large="google/flan-t5-large" # 800 Mio Parameters
    # eilamc14/t5-base-text-simplification # 200 Mio Parameters
...
@app.cell
def _(AutoTokenizer, asset_folder_path, flan_t5_large, load_asset_data, mo):
    tokenizer = AutoTokenizer.from_pretrained(flan_t5_large, legacy=False)
...
def _(mo):
    ...
    mrm8488 model  (~60M)
    * Learning Rate: 1e-4 -> Small models need higher learning rate to update weights
    * Batch Size: 16 -> Moderate batch size to allow exploration of creative simplificiations
    * Epochs: 8 -> Has fewest parameters and therefore needs more phases to learn
    * Warmup Ratio: 0.1

    Flan-T5 Small  (~80M)
    * Learning Rate: 1e-4 -> Provides signal to adapt wights to simplification
    * Batch Size: 16 -> Moderate batch size to allow exploration of creative
    * Epochs: 5 -> Instruction-tuned models need less epochs to reach good performance
    * Warmup Ratio: 0.1

    Flan-t5 Large (~780M)
    * Learning Rate: 3e-5 -> Lower learning rate to prevent "forgetting" of pre-trained knowledge
    * Batch Size: 16 -> Balance between stable gradient estimates and stochastic noise for a large model
    * Epochs: 3 -> Training for high amounts of epochs risks overfitting
    * Warmup Ratio: 0.1
    """)
    return
...
def _(
    ...
    # Load model
    model_t5 = T5ForConditionalGeneration.from_pretrained(flan_t5_small)
    # Training arguments
...
@app.cell
def _(torch):
    def simplify_with_mrmm_t5(text, model, tokenizer, device, max_length=128):
        model.eval()
        ...
        with torch.no_grad():
            outputs = model.generate(
                input_ids,
                max_new_tokens=64,
                min_length=10,           # Prevent model from just deleting text
                num_beams=4,             # Stable beam search
                length_penalty=0.4,      # More aggressive shortening for T5-Small
                no_repeat_ngram_size=3,  # Critical to prevent repetition loops
                early_stopping=False
            )
        return tokenizer.decode(outputs[0], skip_special_tokens=True)
    return (simplify_with_mrmm_t5,)


@app.cell
def _(torch):
    def simplify_with_flan_t5_small(text, model, tokenizer, device, max_length=128):
        model.eval()
        input_text = "Simplify: " + text
        ...
        with torch.no_grad():
            outputs = model.generate(
                ...
                num_beams=5, 
                do_sample=False, 
                length_penalty=0.4,      # Slightly favor shorter text
                early_stopping=False,
            )
        return tokenizer.decode(outputs[0], skip_special_tokens=True)
    return (simplify_with_flan_t5_small,)


@app.cell
def _(torch):
    def simplify_with_flan_t5_large(text, model, tokenizer, device, max_length=128):
        model.eval()
        input_text = "Simplify: " + text
        ...
        with torch.no_grad():
            outputs = model.generate(
                input_ids,
                max_new_tokens=64,       # Limits output words specifically
                num_beams=5,             # Higher beams for high-quality paths
                do_sample=False,         # Deterministic for reliability
                length_penalty=0.0,      
                early_stopping=False,
            )
        return tokenizer.decode(outputs[0], skip_special_tokens=True)
    return (simplify_with_flan_t5_large,)


@app.cell
def _(torch):
    def simplify_with_eilamc_t5(text, model, tokenizer, device, max_length=128):
        model.eval()
        ...
        # The parameters for this model were taken directly from the documentation on Huggingface
        with torch.no_grad():
            outputs = model.generate(
                input_ids,
                max_new_tokens=64,      
        ...
        return tokenizer.decode(outputs[0], skip_special_tokens=True)
    return (simplify_with_eilamc_t5,)
...
def _(
    ...
    mo,
    mrmm_model,
    simplify_with_eilamc_t5,
    simplify_with_flan_t5_large,
    simplify_with_flan_t5_small,
    simplify_with_mrmm_t5,
    test_input,
):
    # 1. Define the path to checkpoint
    mrmm_checkpoint = './checkpoints/mrm8488_checkpoint-4200'
    flan_t5_small_checkpoint = "./checkpoints/flan_t5_small_checkpoint-3000"
    flan_t5_large_checkpoint = "./checkpoints/flan_t5_large_checkpoint-3600"

    # 2. Load the specific fine-tuned weights 
    mrmm_t5_model = T5ForConditionalGeneration.from_pretrained(mrmm_checkpoint).to(device_t5)
    flan_t5_small_model = T5ForConditionalGeneration.from_pretrained(flan_t5_small_checkpoint).to(device_t5)
    flan_t5_large_model = T5ForConditionalGeneration.from_pretrained(flan_t5_large_checkpoint).to(device_t5)

    # 3. Use the loaded model in  generation step
    input_text_test = test_input.value.strip()

    # mrmm
    tokenizer_mrmm = AutoTokenizer.from_pretrained(
        mrmm_model, legacy=False
    )
    simplified_mrmm = simplify_with_mrmm_t5(
       input_text_test, mrmm_t5_model, tokenizer_mrmm, device_t5
    )
    # flan_t5_small
    tokenizer_flan_t5_small = AutoTokenizer.from_pretrained(
        flan_t5_small, legacy=False
    )
    simplified_flan_t5_small = simplify_with_flan_t5_small(
        input_text_test, flan_t5_small_model, tokenizer_flan_t5_small, device_t5
    )
    # flan_t5_large
    tokenizer_flan_t5_large = AutoTokenizer.from_pretrained(
        flan_t5_large, legacy=False
    )
    simplified_flan_t5_large = simplify_with_flan_t5_large(
        input_text_test, flan_t5_large_model, tokenizer_flan_t5_large, device_t5
    )
    ...
    # Simplify with pre-trained model without fine-tuning
    simplified_eilamc_t5 = simplify_with_eilamc_t5(
    ...
    **Original (Complex):**
    ...
    |-------|-------------------|
    | **mrm8488/t5-small-finetuned-text-simplification** | {simplified_mrmm} |
    | **google/flan-t5-small** | {simplified_flan_t5_small} |
    | **google/flan-t5-large** | {simplified_flan_t5_large} |
    | **eilamc14/t5-base-text-simplification** | {simplified_eilamc_t5} |
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Short info on the length penalty parameter:
    |Value|Effect on Output|Rationale|
    | :---| :--- | :--- |
    |-1.0|Very Short|Aggressively penalizes every additional token generated.|
    |0.0|Short|No length normalization; the model favors shorter sequences to avoid accumulating more negative log-probabilities.|
    |1.0|Natural|Standard normalization; the neutral baseline where the model follows its training distribution.|
    """)
    return
...
def _(AutoTokenizer, T5ForConditionalGeneration, torch):
    def _():
        ...
        CHECKPOINT_PATH = "./checkpoints/flan_t5_large_checkpoint-3600" # Your local fine-tuned model
        ...
        def simplify_with_flan_t5_large_live(text):
            model.eval()
            input_text = "Simplify: " + text
        ...
        # def simplify_with_flan_t5_small_live(text):
        #     model.eval()
        #     input_text = "Simplify: " + text
        ...
        # def simplify_with_eilamc_t5_live(text):
        #     model.eval()
        #     input_text = "Simplify: " + text
        ...
        hyps = [simplify_with_flan_t5_large_live(text) for text in tqdm(gold_orig)]
        ...
        output_file = "./trialdata_results/0.0_flan_t5_large_validation_results.csv"
